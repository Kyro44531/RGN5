...
sr = 250000;                          % OFDM符号速率
fd = 1000;                             % 最大多普勒频移
ts = 1/sr/N_symbo;                         % OFDM符号抽样时间间隔
t=0:ts:(N_symbo*(Nd+1))*ts*59;              % 抽样时刻
...
Ber=zeros([5,length(SNR)]);                                          % 译码后
Ber2=zeros([5,length(SNR)]);     

% 放在了for循环里面，来检测不同的信道的情况


% 译码前

%%  New 直接把jj的跨度改大，这样出来的图会少一点
for fd = 0:800:3200
    figure
...
%% New 三径的数据
Tx_data2 = zeros(1,length(Tx_data));
Tx_data2(5:end) = Tx_data(1:end-4);
Tx_data3 = zeros(1,length(Tx_data));
Tx_data3(9:end) = Tx_data(1:end-8);

% 三径的信道参数
hh1 = h1(1:N_symbo*(Nd+1)*59);
hh2 = h2(1:N_symbo*(Nd+1)*59);
hh3 = h3(1:N_symbo*(Nd+1)*59);

% 三径的【每一帧】数据
dd1=Tx_data(1:N_symbo*(Nd+1)*59); %取出第1径每一帧的数据
dd2=Tx_data(1:N_symbo*(Nd+1)*59); %取出第2径每一帧的数据
dd3=Tx_data(1:N_symbo*(Nd+1)*59); %取出第3径每一帧的数据

for jj = 1:8:41
    %rx_channel = awgn(Tx_data,SNR(jj),'measured');    % 添加高斯白噪声
    % New 此处改为多径信道

    spow1=norm(data,'fro').^2/(N_sc*Nd);        % 计算符号能量
    sigma1=sqrt(1/2*spow1/log2(M)*10.^(-jj/10)); % 根据SNR计算噪声标准差
    rx_channel_0 = hh1.*dd1+hh2.*dd2+hh3.*dd3;
    rx_channel = awgn(rx_channel_0,jj,'measured');
    %信号经过三径衰落信道，并加入高斯白噪声

%% 串并转换
    Rx_data1 = reshape(rx_channel,N_symbo,[]);
...
%% 解扩
    ...
    xlim([-1.2,1.2]);
    ylim([-1.2,1.2]);
    title(sprintf("信噪比为%d,最大多普勒频移为%d时的接收点星座图", jj,fd));
...
%% 计算误码率
    [~,Ber2(fd/800+1,jj)] = biterr(De_Bit(1:length(code_data)),code_data);% 译码前的误码率
    [err, Ber(fd/800+1,jj)] = biterr(rx_c_de(1:length(P_data)),P_data);% 译码后的误码率
