...
% described in TS 38.213 Tables 13-11 and 13-12 [ <#18 5> ] .
...
monSymbols = monSymbols+symbolsPerSlot*K_0;

if K_0 > 0    
    % Display the OFDM grid of the slot containing associated PDSCH
    figure;
    imagesc(abs(rxGrid(csetSubcarriers,monSymbols,1))); axis xy
    xlabel('OFDM symbol');
    ylabel('subcarrier');
...
subsPdschSym = double(nrPDSCHIndices(c0Carrier,pdsch,'IndexStyle','subscript'));
subsPdschDmrs = double(nrPDSCHDMRSIndices(c0Carrier,pdsch,'IndexStyle','subscript'));
